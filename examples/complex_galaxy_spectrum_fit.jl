...
function fitted_components(cm, p, lambda_obs)
    ...
    (; z, continuum, balmer, broad_balmer, forbidden, absorption)
end
...
report(name, truth_value, fit_value) =
    println(rpad(String(name), 22), "truth = ", rpad(round(truth_value; digits = 5), 12),
            "fit = ", round(fit_value; digits = 5))

report(:redshift_z, slot(paramnames(truth_cm), params(truth_cm), :redshift_z),
       slot(names, sol.u, :redshift_z))
report(:ha_amplitude, slot(paramnames(truth_cm), params(truth_cm), :ha_amplitude),
       slot(names, sol.u, :ha_amplitude))
report(:ha_sigma, slot(paramnames(truth_cm), params(truth_cm), :ha_sigma),
       slot(names, sol.u, :ha_sigma))
report(:broad_ha_amplitude, slot(paramnames(truth_cm), params(truth_cm), :broad_ha_amplitude),
       slot(names, sol.u, :broad_ha_amplitude))
report(:broad_ha_sigma, slot(paramnames(truth_cm), params(truth_cm), :broad_ha_sigma),
       slot(names, sol.u, :broad_ha_sigma))
report(:oiii_b_amplitude, slot(paramnames(truth_cm), params(truth_cm), :oiii_b_amplitude),
...
scatter!(zax, lambda[zoom_data], flux[zoom_data]; color = (:gray35, 0.75),
         markersize = 5, label = "data")
lines!(zax, lambda_plot[zoom_plot], truth_plot[zoom_plot]; color = (:black, 0.35),
       linestyle = :dash, linewidth = 1.8, label = "truth")
lines!(zax, lambda_plot[zoom_plot], fit_plot[zoom_plot]; color = :black,
       linewidth = 2.8, label = "best fit")
lines!(zax, lambda_plot[zoom_plot], parts.continuum[zoom_plot]; color = :gray35,
       linestyle = :dot, linewidth = 2.0, label = "continuum")
lines!(zax, lambda_plot[zoom_plot], parts.continuum[zoom_plot] .+ parts.balmer[zoom_plot];
       color = :crimson, linewidth = 2.2, label = "narrow Halpha")
lines!(zax, lambda_plot[zoom_plot], parts.continuum[zoom_plot] .+ parts.broad_balmer[zoom_plot];
       color = :purple3, linewidth = 2.4, label = "broad Halpha")
lines!(zax, lambda_plot[zoom_plot], parts.continuum[zoom_plot] .+ parts.forbidden[zoom_plot];
...
for (i, (label, lambda0)) in enumerate(line_labels)
    ...
    text!(ax, lambda_shifted, y_label; text = label, fontsize = 8,
...
for (i, (label, lambda0)) in enumerate((("[NII]", L_NII_B), ("Halpha", L_HA), ("[NII]", L_NII_R)))
    ...
    text!(zax, lambda_shifted, maximum(fit_plot[zoom_plot]) * (1.02 + 0.045 * (i - 1));
