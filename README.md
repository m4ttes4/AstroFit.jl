...
AstroFit uses a single annotated model tree. Operator nodes (`Sum`, `Pipe`, …)
are the compound types; each named leaf is a `Leaf{name}` carrying the
component's model values plus a positional tuple of constraints (one per field).
`CompiledModel` holds this tree (and priors, once implemented).

Constraints are type-encoded (`Free`, `Fixed`, `Bounded`, `Tied`). Their position
in the leaf's tuple identifies the parameter — no stored index. `withparams` is
`@generated` and assigns slots by a compile-time tree walk, emitting `p[k]` as
literals. This means editing a constraint is a local swap with no renumbering.

The generated code produces straight-line struct rebuilds with no dynamic dispatch
and no heap allocation beyond the new model structs. ForwardDiff compatibility is
automatic: `Dual` values flow through the same straight-line code path.
